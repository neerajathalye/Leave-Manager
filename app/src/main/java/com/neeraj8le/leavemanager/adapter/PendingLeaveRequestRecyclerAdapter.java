...
public class PendingLeaveRequestRecyclerAdapter extends RecyclerView.Adapter<PendingLeaveRequestViewHolder> {

    Context context;
    ArrayList<Leave> leaves;

    public PendingLeaveRequestRecyclerAdapter(Context context, ArrayList<Leave> leaves) {
        this.context = context;
        this.leaves = leaves;
    }

    @Override
    public PendingLeaveRequestViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.pending_leave_request_item, parent,false);
        return new PendingLeaveRequestViewHolder(view);
    }

    @Override
    public void onBindViewHolder(final PendingLeaveRequestViewHolder holder, int position) {
...
//                        leaves.get(position).getApplicationDate(), leaves.get(position).getId());
                ...
                intent.putExtra("intent_data_key","pending");
